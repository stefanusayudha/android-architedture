...
suspend fun getTodoList(payload: GetTodoListPayload): List<Todo> {
    return todoListWebApi.getTodoList(payload.getQueryMap())
}

suspend fun getTodoById(payload: GetTodoByIdPayload): Todo {
    return todoListWebApi.getTodoById(payload.id)
}